...
func NewEngine(config *Config, ginMiddlewareCollection *GinMiddlewareCollection) *gin.Engine {
	...
	for _, ginMiddleware := range *ginMiddlewareCollection {
		engine.Use(ginMiddleware.Provide())
	}
	return engine
}
